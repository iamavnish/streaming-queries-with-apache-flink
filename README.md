...
For above use case, three tables are created. One table (kinesis-input-stream) is connected to data source and remaining two tables (output_sliding_sales_stream, output_device_sliding_stream) are connected to data sinks. Both, data source and data sinks are Amazon Kinesis Data Streams. For data processing, a sliding window table is created with a window size of 6 minutes and window slide of 3 minutes and a sliding window query is executed to get the total number of sales.
